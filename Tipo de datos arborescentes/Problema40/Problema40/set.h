...
protected:
    ...
    std::pair<bool, T> lower_bound(T const& e, Link a) const {
        ...
        if (a == nullptr) return { false,e };
        else if (e < a->elem) {
            if (a->iz != nullptr ) {
                T hijoIzq = a->iz->elem;
                
                std::pair <bool, T> izqSol = lower_bound(e, a->iz);

                if (izqSol.first) return izqSol;  // Buscamos si en el hijo izquierdo del hijo izquierdo existe algun mayor
            }
            
            return { true,a->elem };
        }
        else if (e > a->elem) {  // buscamos elemento si hay elemento mayor que el por la derecha

            if (a->dr != nullptr) {
                if (e > a->dr->elem || e < a->dr->elem) return lower_bound(e, a->dr);
                else return { true,a->dr->elem };
            }
            else return { false,e };
        }
        else return { true,e }; // e == a->elem
